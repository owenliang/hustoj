...
function findProblemWithTitle($title) {
  //return false;	
  $md5 = md5($title);
  $sql = "SELECT problem_id FROM problem WHERE md5(title)=?";
  $result = pdo_query($sql, $md5);
  if(empty($result)) {
    return 0;
  }
...
function import_fps($tempfile)
{
  ...
  foreach ($searchNodes as $searchNode) {
    ...
    /*if (!hasProblem($title)) {*/
    $pid=findProblemWithTitle($title);
    if ($pid) {
      updateProblem($pid,$title, $time_limit, $memory_limit, $description, $input, $output, $sample_input, $sample_output, $hint, $source, $spj, $OJ_DATA);
    } else {
    ...
    foreach ($images as $img) {
      ...
      if (!in_array($src, $did)) {
        ...
        // 相同base64的图片在1道题目下只存储一份
        $newpath=$domain . '/' . $pid . '/' . md5($base64) . $ext;
        if ($OJ_SAE)
